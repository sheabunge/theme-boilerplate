...
/**
 * Displays the page meta information
 *
 * @since      0.1
...
<div class="loop-meta">
	...
	<?php } elseif ( is_search() ) { ?>
		...
		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the search results for "%s"', 'theme-boilerplate' ), esc_attr( get_search_query() ) ) ); ?>
		</div><!-- .loop-description -->
	...
	<?php } elseif ( is_day() || is_month() || is_year() ) {
			...
			if ( is_day() )
				$date = get_the_time( __( 'F d, Y', 'theme-boilerplate' ) );
			elseif ( is_month() )
				$date = get_the_time( __( 'F Y', 'theme-boilerplate' ) );
			elseif ( is_year() )
				$date = get_the_time( __( 'Y', 'theme-boilerplate' ) );
		?>

		<h1 class="loop-title"><?php echo $date; ?></h1>

		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the site archives for %s.', 'theme-boilerplate' ), $date ) ); ?>
		</div><!-- .loop-description -->

	<?php } elseif ( is_archive() ) { ?>

		<h1 class="loop-title"><?php _e( 'Archives', 'theme-boilerplate' ); ?></h1>

		<div class="loop-description">
			<?php echo wpautop( __( 'You are browsing the site archives.', 'theme-boilerplate' ) ); ?>
		</div><!-- .loop-description -->
